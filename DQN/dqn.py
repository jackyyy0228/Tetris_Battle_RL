...
class Estimator():
    ...
    def _build_model(self):
        ...
        conv1 = tf.contrib.layers.conv2d(
            X1, 32, 3, activation_fn=tf.nn.relu)
        conv2 = tf.contrib.layers.conv2d(
            conv1, 32, 3, activation_fn=tf.nn.relu)
        conv3 = tf.contrib.layers.conv2d(
            conv2, 64, 3, activation_fn=tf.nn.relu)
    ...
    def update(self, sess, X1, X2, a, y):
        """
        ...
        """
        feed_dict = { self.grid_states: X1,self.piece_states: X2, self.y_pl: y, self.actions_pl: a }
...
def deep_q_learning(sess,
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
            next_state, reward, done, line_sent = env.step(VALID_ACTIONS[action])
            ...
            loss = q_estimator.update(sess, states1_batch,states2_batch, action_batch, targets_batch)
...
experiment_dir = os.path.abspath("./experiments/{}".format('tetris'))
